...
std::vector<int> simulateZ2(NumericVector beta_by_region,
  ...
  for (int i = 1; i < n; i++) {
    ...
    if (seqnames_one_tuples[i] != seqnames_one_tuples[i - 1]) {
      ...
      // UP TO HERE: Figure out why methsim::ipf() breaks .simulateZ(). Try 
      // calling methsim::ipf() from both R and C++ multiple times in a 
      // for-loop (e.g., N = 1,000,000) and see if problems arise.
      // FIXME: This line causes the bug; possibly related to 
      ...
      joint_prob_matrix = methsim::ipf(ipf_seed, row_margins, col_margins,
                                       1000, 1e-10);
      // In constrast, manually constructing joint_prob_matrix works without 
      // error. So it's a problem with methsim::ipf().
//      joint_prob_matrix << 0.20 << 0.04 << arma::endr
//                        << 0.09 << 0.67 << arma::endr;
