...
- [x] Being able to simulate a single truth (`simulate,SimulateMethylomeParam-method`).
- [x] Being able to simulate reads from a single truth (`simulate,SimulateBSParam-method`).
- [x] Being able to generate `MethPat` object(s) from a single set of simulated reads (`asMethPat()`).
...
- Simulate a methylome with a single haplotype (i.e., $w_{1} := 1$). This should result in it having the "correct" co-methylation structure but "incorrect" beta-values, I think.
- Simulate a methylome with many pseudo-haplotypes ($\approx 20$?) and use uniform `W`. See what sort of behaviour we get.
